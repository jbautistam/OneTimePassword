...
temporal para una sesión. Este tipo de contraseña va a utilizarse una sola vez y caduca después de un breve período de tiempo. 

Este tipo de algoritmos se utilizan para agregar una capa adicional de seguridad en diversas aplicaciones permitiendo la autentificación 
...
* solo sea accesible para su dueño.
* **Generación segura:** creadas mediante procesos criptográficos que imposibilita la predicción para los atacantes.
...
* **Clave secreta compartida:** Ambas partes (el servidor y el dispositivo de autenticación) comparten una clave secreta que se utiliza para generar las contraseñas OTP.
* **Contador:** Un contador se incrementa cada vez que se genera una nueva contraseña OTP. Este contador asegura que cada contraseña OTP sea única.
...
* **Generación de OTP:** La contraseña OTP se genera combinando la clave secreta y el valor del contador, y luego truncando el resultado en un formato más amigable para el usuario, generalmente un número de 6 a 8 dígitos.

**Proceso de funcionamiento del algoritmo HOTP:**

* **Inicialización:** El servidor y el dispositivo de autenticación acuerdan una clave secreta y un contador inicial.
* **Generación de OTP:** Cuando se necesita una contraseña OTP, el dispositivo de autenticación combina la clave secreta y el valor del contador actual, y aplica la función HMAC para generar una contraseña OTP.
* **Truncamiento:** La salida de la función HMAC se trunca para obtener una contraseña OTP de 6 a 8 dígitos.
* **Incremento del contador:** Después de usar la contraseña OTP, el contador se incrementa en una unidad tanto en el servidor como en el dispositivo de autenticación.
* **Autenticación:** El servidor verifica la contraseña OTP proporcionada por el usuario generando una contraseña OTP localmente con la clave secreta y el contador actual, y comparándola con la proporcionada por el usuario.
...
**Ventajas del algoritmo HOTP:**
	...
	* **Flexibilidad:** Puede ser utilizado en una variedad de aplicaciones, incluyendo autenticación en línea y transacciones financieras.
**Desventajas del algoritmo HOTP:**
	* **Problema de sincronización:** Si los contadores del servidor y el dispositivo de autenticación se desincronizan, puede requerir un protocolo de resincronización para resolver el problema.
	* **No tiene período de expiración:** Las contraseñas OTP generadas por HOTP permanecen válidas hasta que se utilice la siguiente, lo que puede ser un problema de seguridad si no se utiliza inmediatamente.
...
* **Clave secreta compartida:** ambas partes (el servidor y el dispositivo de autenticación) comparten una clave secreta que 
...
* **Inicialización:** el servidor y el dispositivo de autenticación acuerdan una clave secreta y un tiempo inicial (generalmente el tiempo Unix).
* **Generación de OTP:** cuando se necesita una contraseña OTP, el dispositivo de autenticación combina la clave secreta y la hora actual
...
* **Usabilidad:** el algoritmo TOTP es fácil de implementar y utilizar, especialmente con aplicaciones de autenticación como 
Authy y Google Authenticator.

**Desventajas del algoritmo TOTP:**
* **Problema de sincronización:** si los relojes del servidor y el dispositivo de autenticación no están sincronizados, 
...
Una vez inicializa la clase, simplemente llamando al método `Compute` con el contador, obtendremos el código de validación:
